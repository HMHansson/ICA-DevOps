...
#############################################################################
##                                                                          #
## exportAPIs.sh : Export all Rest and SOAP APIs for a project              #
##                                                                          #
#############################################################################
wmio_endpoint=$1
wmio_user=$2
...
for item in $(jq  -c -r '.output.rest_api[]' <<< "$projectListJson"); do
  exportRestAPI ${wmio_endpoint} ${wmio_user} ${wmio_password} ${wmio_projectName} $item
done

# Exporting SOAP APIs
for item in $(jq  -c -r '.output.soap_api[]' <<< "$projectListJson"); do
  exportSoapAPI ${wmio_endpoint} ${wmio_user} ${wmio_password} ${wmio_projectName} $item
done
